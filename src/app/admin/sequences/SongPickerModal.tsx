    ...
    return (
      <div
        ...
        style={{
          ...
          padding: '10px 14px',
          borderRadius: '12px',
          background: isAlreadyInSeq ? 'rgba(30,58,138,0.02)' : 'rgba(255, 255, 255, 0.9)',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', flex: 1, minWidth: 0 }}>
          ...
          >
            {isAlreadyInSeq ? <Check size={16} /> : <Music size={16} />}
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              ...
              {isAlreadyInSeq && (
                <span style={{ fontSize: '0.66rem', fontWeight: 700, color: 'var(--success)', background: 'rgba(16,185,129,0.1)', padding: '1px 7px', borderRadius: '10px' }}>
                  In Sequence
                </span>
              )}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          <button
            ...
            style={{ minHeight: '36px', padding: '4px 10px', fontSize: '0.78rem', gap: '4px' }}
    ...
    >
      <div
        style={{
          background: '#ffffff',
          borderRadius: '24px',
          boxShadow: '0 25px 50px -12px rgba(11, 77, 36, 0.25)',
          maxWidth: '960px',
          width: '100%',
          maxHeight: '90vh',
          display: 'flex',
      ...
      >
        ...
        >
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--primary)', margin: 0 }}>
                Select Song for "{sequenceTitle}"
              </h2>
            </div>
            <p style={{ color: 'var(--muted)', fontSize: '0.82rem', margin: '2px 0 0' }}>
              Organized by Mass Parts Repertoire & A-Z Hymnal Index
            </p>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <button
              ...
              onClick={onClose}
              className="btn btn-secondary"
              style={{ minWidth: '38px', height: '38px', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              <X size={18} />
            </button>
          </div>
        ...
        {/* Filter & Search Bar */}
        <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(0,0,0,0.06)', display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center', background: '#fafafa' }}>
          {/* Search Input */}
          <div style={{ position: 'relative', flex: '1 1 240px', minWidth: '220px' }}>
            ...
            <input
              ...
              style={{ paddingLeft: '36px', paddingRight: searchValue ? '36px' : '14px', width: '100%', minHeight: '40px', fontSize: '0.85rem' }}
            />
            {searchValue && (
              <button
                type="button"
          ...
          <div style={{ flex: '0 0 auto', minWidth: '200px' }}>
            <select
              ...
              style={{ width: '100%', minHeight: '40px', fontSize: '0.82rem', fontWeight: 600 }}
            >
          ...
          {(searchValue || selectedCategoryFilter !== 'ALL' || selectedCategoryTags.length > 0) && (
            <button
              ...
              style={{
                ...
                color: 'var(--muted)', background: 'rgba(0,0,0,0.05)', border: 'none', minHeight: '40px', cursor: 'pointer',
              }}
            >
              Clear Filter
            </button>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
          ...
          {filteredSongs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--muted)' }}>
              <Music size={38} style={{ margin: '0 auto 10px', opacity: 0.5 }} />
              <p style={{ margin: 0, fontSize: '0.92rem' }}>No matching songs found in repertoire.</p>
            </div>
          ) : viewMode === 'AZ_INDEX' ? (
                ...
                return (
                  <div key={letter} style={{ borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--glass-border)', background: '#fafafa' }}>
                    <div style={{ padding: '8px 14px', background: 'rgba(30,58,138,0.06)', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--primary)' }}>{letter}</span>
                      <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--muted)' }}>{letterSongs.length} songs</span>
                    </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        {isCollapsed ? <ChevronDown size={17} style={{ color: 'var(--muted)' }} /> : <ChevronUp size={17} style={{ color: 'var(--muted)' }} />}
                      </div>
                    </div>
                    ...
                    {!isCollapsed && (
                      <div style={{ padding: '10px 14px 14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                            ...
                            {sectionCategoryTags.length > 0 ? (
                              sectionCategoryTags.map((tag) => (
                                <span
                                  key={tag}
                                  style={{
                                    fontSize: '0.7rem', fontWeight: 700, padding: '1px 6px', borderRadius: '8px',
                              ...
                              <span style={{ fontSize: '0.7rem', fontStyle: 'italic', color: 'var(--muted)' }}>None</span>
                            )}
                          </div>
                        ...
                        {paginatedPartSongs.length > 0 ? (
                          ...
                          <p style={{ fontSize: '0.8rem', color: 'var(--muted)', fontStyle: 'italic', padding: '6px 0' }}>No songs in this Mass Part section.</p>
                        )}
                      </div>
                    )}
                  </div>
              ...
              {/* Other Songs Section */}
              {otherSongs.length > 0 && (
                <div style={{ borderRadius: '14px', border: '1px solid var(--glass-border)', background: '#ffffff', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                  <div
                    onClick={() => toggleSection('other-songs')}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', cursor: 'pointer', background: 'rgba(0,0,0,0.02)', borderRadius: collapsedSections['other-songs'] ? '14px' : '14px 14px 0 0' }}
                  >
                    <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--primary)', margin: 0 }}>Other Repertoire Songs</h3>
                    <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--muted)' }}>{otherSongs.length} songs</span>
                  </div>
        ...
        {/* Modal Footer */}
        <div style={{ padding: '14px 24px', borderTop: '1px solid rgba(0,0,0,0.08)', background: '#fafafa', display: 'flex', justifyContent: 'flex-end' }}>
          <button type="button" onClick={onClose} className="btn btn-secondary" style={{ minHeight: '40px', padding: '8px 20px', fontSize: '0.88rem' }}>
            Done / Close
          </button>
        </div>
      </div>
